...
module.exports = function(qube, slatejs) {
...
function _include_expressions(node) {
	if (!node.attributes || !node.attributes.include) return [];
	...
	try {
		var imported = ot.parse(node.attributes.include)[0];
		var ret = imported.qube_expressions();
		var url = node.values[1].textContent();
		if (/ +as +/.test(url)) {
			var namespace = url.split(/ +as +/)[1].trim();
			var nret = ['Namespace', sym(namespace)];
			Array.prototype.push.apply(nret, ret);
			...
			*/
			...
			categoryColumns.forEach(function(col) {
				...
				body.forEach(function(row) {
					...
					if(unique.indexOf(content) == -1) {
						unique.push(content);
						var expr = str(content);
				...
				body.forEach(function(row) {
					...
					try {
						...
						for (var kcol in keys) {
							...
							if (cont !== '')
								expr = ['LetG', sym(key), str(cont), expr];
						}
				...
				body.forEach(function(row) {
					...
					try {
						...
						for (var kcol in keys) {
							...
							var cont = row.values[1+kcol].textContent();
							if (cont !== '')
								expr = ['LetS', sym(key), str(cont), expr];
						}
...
List.prototype.qube_expressions = function() {
	...
	switch (this.head().sym) {
		...
		case 'import':
			...
			break;
		case 'include':
			this._qube_expressions = _include_expressions(this);
			break;
		case 'cache':
			this._qube_expressions = this.tail();
			break;
		case 'result':
			...
			this.values.forEach(function(x,i) {
				if (typeof x.qube_expressions === 'function'
					...
					this._qube_expressions = this._qube_expressions.concat(xs)
				} 
			},this);
			break
		default:
...
function map_result(res, cell) {
	if (res === undefined || res === null) {
		...
		return r;
	} else if (typeof res === 'object' && res.type === 'cache') {
		var r = new List();
		r.push(_sym('cache'));
		r.push(res.expr);
		return r;
	} else if (typeof res === 'object' && res.type === 'action') {
		var r = new List();
		r.push(_sym('expr'));
		r.push(cell.sexpr);
		return r;
...
function map_outs(results) {
	...
	while ((cell = results.pop())) {
		if (cell.errors.length > 0) {
			...
			try {
				var x = map_result(cell.result(), cell);
...
List.prototype.replace_results = function(ops, results) {
	...
	if ((exprs.length === 0 && !(this.has_results())) || this.head().sym == 'include' || this.head().sym == 'import') {
...
var Import = createClass({
	fetched: function(response) {
		...
		editor.apply(op);
		editor.run_command('recalculate');
	},
...
//include puts include in attribute
var Include = createClass({
	fetched: function(response) {
		...
		var ops = ot.operations;
		var op = [];
		op.push(ops.retain(x.offset));
		var unattributes;
		if (x.node.attributes && x.node.attributes.include)
			unattributes = {include: x.node.attributes.include};
		var attributes = {include: response};
		op = op.concat(ops.retain(1,attributes, unattributes));
		editor.apply(op);
		editor.run_command('recalculate');
	},
	error: function(error) {
	...
	render: function() {
		...
		try {
			if (attributes.include) {
				imported = ot.parse(attributes.include)[0];
			}
		} catch (e) {
			console.log(e);
		}
		var url = obj.values[1];
		var action_class = 'right-icon';
		var action = this.download;
		var fa_action;
		var title = '';
...
var Cache = createClass({
	render: function() {
		var obj = this.props.obj;
		var cache = ql.showMr(obj.index(1));
		return DOM.pre({},[DOM.text(cache)]);
	}
});
...
function render_result(res) {
	if (Array.isArray(res) && res.length > 0) {
		if (Array.isArray(res[0])) {
			...
			if (a.headers) {
				...
				a.headers.forEach(function(h) {
					var ats = (a.dimensions.indexOf(h) >= 0) ?
						{className:'category'} : {};
					attributes.push(ats);
					cs.push(DOM.th({className: ats.className},[render_result(h)]));
				});
				hr.push(DOM.tr({},cs));
			}
			//TODO: slice off first 1000 and show a foot
			// of how many we are not going to show...
			br = a.map(function(r) {
				var nr = DOM.tr({},r.map(function(c,i) {
					var ats = {};
					if (attributes[i])
						ats.className = attributes[i].className;
					return DOM.td(ats,[render_result(c)]);
				}));
				return nr;
			});
			...
			br = a.map(function(r) {
				var nr = DOM.tr({},[
					DOM.td({className: ats.className},
						[render_result(r)])]);
				return nr;
			});
			return DOM.table({className:'pure-table pure-table-bordered'},
		...
		return DOM.a({onClick:f,href:'#'},[DOM.text(res.name)]);
	} else {
		return DOM.text(res.toString());
	}
}
...
var Expression = createClass({
	render: function() {
		var obj = this.props.obj;
		var editor = this.props.editor;
		var sexpr = obj.index(1);
		var res = editor.run_command('seval',sexpr);
		var r = render_result(res);
		return r;	
	}
});
...
renderers.cache = Cache;
renderers.expr = Expression;
...
var functions = {
	Cache: function(expr, reset_expr) {
		return 	['Call', ['Symbol', '_Cache'], expr, ['JSON', expr], 
				reset_expr,	['JSON', reset_expr]];
	},
	_Cache: function(expr, sexpr, reset, reset_sexpr) {
		return {
			type:'cache',
			expr: ['Set', ['Slice', sexpr, ['Equal', reset_sexpr, ['JSON', reset]]], ['JSON', expr]],
		};
	},
	Download: function(filename, func) {
		return {
			type:'action',
			name: filename,
			func: function() {
				res = func()
				console.log(res);
				fs.saveAs(new Blob([res]), filename);
			},
		};
	},
}
functions.Cache.isMacro = true;
//functions.Download.isMacro = true;

	qube.extend(functions);
	...
	return function(editor) {
		...
		qube.onRecalculate = refresh;
	
		function seval(sexpr) {
			try {
				return qube.eval(sexpr);
			}
			catch(err) {
				return ['Error', err.toString()];
			}
		}
		seval.description = function(sexpr) {
			return "Evaluates " + JSON.stringify(sexpr);
		};
